...
- **Runtime/Data 专项场景**：框架侧新增 `res://SlimeAI/Scenes/Validation/Runtime/Data/RuntimeDataValidation.tscn`，BrotatoLike runner 可作为承载工程运行；该场景覆盖 typed `DataKey<T>` lifecycle、`DataCatalog` resolve、modifier/computed dirty、category reset 和 Data-to-Event bridge artifact。
...
结果：`Tools/run-build.sh` 输出 `Build succeeded. 0 Warning(s) 0 Error(s)`；`run-main-smoke` 输出 `BrotatoLike GameOS smoke PASS` 且 `bridge:True pool:True dataos:True main:True`；analyzer 输出 `status: pass`、`firstError: none`。最新 passing smoke artifact 位于 `.ai-temp/scene-tests/runs/2026-05-15/09-47-27/index.json`。
