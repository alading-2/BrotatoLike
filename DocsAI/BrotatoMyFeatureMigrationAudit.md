...
当前已经从“最小可玩核心链路”推进到“可验证的基础玩家体验闭环”：主场景启动、DataOS snapshot 装载、玩家生成、输入数据写入、玩家位移、第 1 波敌人生成、敌人追逐、接触伤害、死亡清理、`slam` / `chain_lightning` / `target_point_skill` 真实 input action 触发、正式 HUD、头顶血条、四槽技能栏、伤害/治疗飘字、点选指示器、暂停菜单、HP recovery、经验拾取和最小 level-up 反馈都有 Godot scene artifact 证据。

最新 Main 场景 artifact 显示 `BrotatoLike playable slice PASS`，并记录了 `formal_hud_found=True`、`formal_hud_health_text=HP 89/100`、`formal_hud_current_skill=位置目标`、`formal_hud_damage_number_count=77`、`skill_point_report=Success`、`skill_point_targeting_started=True`。专项 Playable UX artifact 显示正式 HUD/血条/技能栏/点选/飘字/可见移动均 pass；Progression artifact 显示 wave completion、pause gate、HP recovery、经验拾取和 level-up 均 pass；LegacyResources artifact 显示 25 个旧 `res://Src/...` / `res://Data/...` path 已分类且没有 missing active legacy path。

这仍不能等价为旧项目所有功能体验迁移完成。剩余缺口主要是：商店 / 道具 / level-up choices / meta progression、更多技能装配到玩家普通局内体验、Chain Lightning 连线视觉、完整多波曲线和波间流程、真实物理设备专项 QA、角色扩展，以及旧测试/调试工具是否按 AI-first 方式重建。
...
- `Games/BrotatoLike/Src/Game/BrotatoLikeGameRuntime.cs` 初始化 DataOS/Spawn schedule、生成玩家、进入 Gameplay、挂载正式 HUD/targeting/progression，并授予玩家 `slam / chain_lightning / target_point_skill / dash` 四个默认技能。
...
- Playable UX artifact：`.ai-temp/scene-tests/runs/2026-05-20/11-32-49/001_Src_Validation_Game_PlayableUX_BrotatoLikePlayableUXValidation.tscn_attempt1/artifacts/brotatolike-playable-ux-validation.json`，`status=pass`，正式 HUD/血条/技能栏/点选/伤害飘字/可见移动均通过。
- Progression artifact：`.ai-temp/scene-tests/runs/2026-05-20/11-33-15/001_Src_Validation_Game_Progression_BrotatoLikeProgressionLoopValidation.tscn_attempt1/artifacts/brotatolike-progression-loop-validation.json`，`status=pass`，wave completion、pause gate、HP recovery、经验拾取、level-up 反馈均通过。
...
- Main artifact：`.ai-temp/scene-tests/runs/2026-05-20/11-33-55/001_Scenes_Main.tscn_attempt1/artifacts/scene-acceptance.json`，`status=pass`，`expectedInputs / expectedObservations / passCriteria / failCriteria / artifactPath` 均非空，`failureReasons=[]`。

## 功能迁移矩阵

| 功能域 | 旧功能意图 | 当前状态 | 缺口判断 | 决策 |
| --- | --- | --- | --- | --- |
| 主场景启动 | 旧 `Main` 发游戏开始事件，系统按事件进入运行态。 | 核心链路和基础体验已迁。普通 Main 初始化 runtime、生成玩家、进入 Gameplay、挂正式 HUD/targeting/progression；smoke/acceptance 分离。 | 商店、道具、完整多波和 meta progression 仍缺。 | Keep：继续以当前 `BrotatoLikeGameRuntime` 为正式入口，不复刻旧场景树。 |
...
| 技能栏与技能槽 | 旧技能栏 4 槽、图标、名称、高亮、冷却遮罩、充能、按键提示。 | 已完成基础体验迁移。正式四槽技能栏显示 owned abilities、选中、高亮、cooldown/charge fallback。 | 图标、复杂充能表现、手柄提示可继续完善。 | Keep。 |
...
| Chain Lightning | 旧连锁闪电技能。 | 核心链路已迁。当前玩家默认拥有，artifact 覆盖目标选择、触发、延迟命中。 | `Ability.LineEffectScenePath` 为空；缺闪电链连线视觉。 | P1 Adopt Now：补线特效和验收。 |
| 其他投射物/位移技能 | 旧数据包含正弦波、回旋镖、贝塞尔、定点抛炸弹、圆弧、冲刺、环绕、护盾。 | 核心 handler/DataOS 已迁；`dash` 已装到玩家四槽技能栏。 | 多数技能仍只在 smoke/handler 路径证明执行；没有全部装配到玩家、技能栏、主场景逐技能可视和命中验收。 | P1 Adopt Later：逐技能装配和验收。 |
| 被动/周期技能 | 旧有永久/周期触发概念。 | 部分迁移。GameOS 有 Periodic/handler 能力，游戏侧部分 handler 已注册。 | 当前玩家默认不会拥有这些技能；普通主场景缺持续环绕/光环/周期伤害体验。 | P1 Adopt Later。 |
...
| 拾取/掉落/经验/升级 | 旧 EnemyData 有 `ExpReward`；旧 PickupComponent 本身也是占位。 | 已完成最小闭环迁移。敌人死亡生成经验拾取，玩家收集后经验/等级更新并有 feedback。 | 经验条样式、升级选项、商店、道具系统和 meta progression 未实现。 | P1 Adopt Later：单独设计完整 roguelite progression。 |
...
1. 完整波次循环：当前已有 wave runtime completion；仍缺多波曲线、WaveStarted/WaveCompleted 对外事件、波间状态、商店/奖励。
2. 更多技能装配：`dash` 已进入玩家四槽；`sine_wave_shot / boomerang_throw / arc_shot / bezier_shot / parabola_shot / orbit_skill / circle_damage / aura_shield` 仍主要是 handler/smoke 证据，需要逐技能主场景可玩验收。
3. 连锁闪电视觉线：补 `LineEffectScenePath`、线段生命周期、延迟弹跳可视。
...
6. 掉落/经验/升级扩展：最小经验拾取与 level-up feedback 已迁；仍缺经验条样式、升级选项、商店、道具和 meta progression。
...
- Handler 注册不等于玩家能用。`BrotatoLikeAbilityHandlers.RegisterAll()` 覆盖多个技能；当前玩家默认四槽是 `slam / chain_lightning / target_point_skill / dash`，其他技能仍需装配和主场景验收。
...
- `expand-brotatolike-skill-loadout`：把剩余投射物/位移/被动技能逐个装配到玩家普通局内体验，并补主场景可视/命中/清理 artifact。
- `restore-brotatolike-chain-lightning-line-vfx`：补连锁闪电线段视觉和生命周期。
- `design-brotatolike-shop-item-levelup-loop`：单独设计商店、道具选择、level-up choices、经济和 meta progression。
