...
最新 `fix-brotatolike-survivor-spawn-input-damage-feedback` targeted batch `.ai-temp/scene-tests/runs/2026-05-21/23-01-36/index.json` 显示 4 个场景全部 PASS。RunFlow artifact 记录开放式持续刷怪 authoring：`wave1_expected_spawn_count=-1`、`wave2_expected_spawn_count=-1`、`wave1_authoring_kind=open`、第一波 spawned count 增长到 `13`、第二波 actual spawned count 为 `12`，同时覆盖 `Completed -> RewardShop`、第 2 波开始、pause gate、死亡复活、cleanup 和 scene-backed wave phase。Main 场景 artifact `.ai-temp/scene-tests/runs/2026-05-21/23-01-36/004_Scenes_Main.tscn_attempt1/artifacts/scene-acceptance.json` 显示 `BrotatoLike playable slice PASS`，记录 `wave_expected_spawn_count=-1`、`wave_actual_spawned_count=16`、`wave_authoring_kind=open`、真实 `SkillSlot1/SkillSlot4` input action path、HUD 与技能栏证据。专项 Playable UX artifact 记录正式 HUD/血条/技能栏/loadout override/点选/池化飘字/可见移动均 pass；Progression artifact 显示 wave completion、pause gate、HP recovery、经验拾取、level-up、经验条和升级三选一均 pass；LegacyResources artifact 显示 25 个旧 `res://Src/...` / `res://Data/...` path 已分类且没有 missing active legacy path。
...
- RunFlow artifact：`.ai-temp/scene-tests/runs/2026-05-21/23-01-36/003_Src_Validation_Game_RunFlow_BrotatoLikeRunFlowValidation.tscn_attempt1/artifacts/brotatolike-run-flow-validation.json`，`status=pass`，DataOS `wave_definition / wave_enemy_entry` authoring、开放式持续刷怪 `ExpectedSpawnCount=-1`、第一波 spawned count 超过首批 5 个敌人、非法 enemy/resource 拒绝、第一波 `Running`、`Completed -> RewardShop`、`shop_offer.validation` hook、第 2 波 `Running`、pause/respawn/cleanup 和 scene-backed wave phase 均通过。
- CharacterSelection artifact：`.ai-temp/scene-tests/runs/2026-05-21/20-00-19/001_Src_Validation_Game_CharacterSelection_BrotatoLikeCharacterSelectionValidation.tscn_attempt1/artifacts/brotatolike-character-selection-validation.json`，`status=pass`，DataOS `character_definition / character_loadout` authoring、非法 player/visual reject、scene-backed CharacterSelectPanelUI、`deluyi/guangfa` 视觉/属性/loadout 差异、selected `player-guangfa` 和玩家绑定均通过。
...
- Main artifact：`.ai-temp/scene-tests/runs/2026-05-21/20-03-04/001_Scenes_Main.tscn_attempt1/artifacts/scene-acceptance.json`，`status=pass`，`expectedInputs / expectedObservations / passCriteria / failCriteria / artifactPath` 均非空，`failureReasons=[]`，记录默认角色 fallback `skill_loadout_source=character:deluyi`、owned ids、visible slot ids、selected id、total count 和 available skill pool。
...
| 玩家生成与视觉 | 旧 PlayerEntity/Preset 组合单位数据、移动、生命、动画、输入。 | 核心链路和首版角色选择已迁。当前 DataOS `unit.player/deluyi` / `unit.player/guangfa` + `character_definition` + `GodotUnitComposer`，Main 默认 fallback 走 `character:deluyi`，专项验证可选择 `guangfa`。 | 已证明 `deluyi/guangfa` 视觉、属性和起始技能差异；`bubing` 等更多资源仍是资产-only，完整角色选择菜单、头像/描述、unlock 和存档选择未实现。 | Keep：首版角色选择完成；更多角色和 meta unlock 后续扩展。 |
...
| 敌人生成 | 旧 SpawnSystem 按波次、计时器、规则生成敌人并发 Wave 事件。 | 已完成首版多波迁移并修正为幸存者式开放刷怪口径。当前 DataOS authoring 使用 open wave / `ExpectedSpawnCount=-1`，RunFlow artifact 证明第一波 spawned count 超过首批 5 个敌人、第二波也持续生成，并覆盖 wave transition、pause/respawn 和 cleanup。 | 更多波次、WaveStarted/WaveCompleted 对外事件、随机/权重策略、boss/特殊事件和完整经济曲线仍缺。 | Keep：保留 DataOS wave authoring 与 RunFlow validation，后续扩展曲线和奖励内容。 |
...
1. 完整波次循环：首版 open wave 两波、`RewardShop` hook、pause/respawn/cleanup 与 scene-backed wave phase 已有 RunFlow artifact；当前普通玩法口径为持续生成、`ExpectedSpawnCount=-1`，仍缺更多波次、随机/权重曲线、WaveStarted/WaveCompleted 对外事件、boss/特殊事件和完整经济交互。
...
- RunFlow：`.ai-temp/scene-tests/runs/2026-05-21/23-01-36/index.json` PASS，per-scene `result.json` exitCode `0`、`firstError=null`，artifact `brotatolike-run-flow-validation.json` 为 `status=pass`、`failureReasons=[]`，记录两波 open authoring、`ExpectedSpawnCount=-1`、第一波 spawned count 超过首批 5 个敌人、非法 enemy/resource 拒绝、第一波 `Running`、`Completed -> RewardShop`、`shop_offer.validation` hook、第 2 波 `Running`、pause/respawn/cleanup 和 scene-backed wave phase。
