...
BrotatoLike 的 `SlimeAI/` 是框架仓 git submodule 镜像。当前初始开发阶段，框架仓新增或修改 `Src/Validation/...`、`Src/Validation/...` 后，BrotatoLike 作为默认承载游戏，直接同步到 `Games/BrotatoLike/SlimeAI/` 工作树以跑通验证。后续多游戏 / 成品阶段不默认同步所有游戏，改按每个游戏的框架版本策略更新 submodule 指针，再选择对应承载游戏跑验证。

`Tools/run-godot-scene.sh` 的 scan roots 必须包含 `Scenes,Src,SlimeAI/Scenes,SlimeAI/Src`，`BrotatoLike.csproj` 必须重新包含 `SlimeAI/Src/Validation/**/*.cs`，否则 submodule 场景脚本不会被编译。
