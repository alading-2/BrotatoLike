...
submodule_path:      /home/slime/Code/SkilmeAI/Games/BrotatoLike/SkilmeAI
...
framework_project:   SkilmeAI/GameOS/SkilmeAI.GameOS.csproj (submodule 内)
...
- 框架源码修改必须切换到框架仓库（`SkilmeAI/` 是只读镜像，禁止在游戏仓直接改动）。
...
- 游戏 csproj 通过 `ProjectReference` 引用 submodule 内 `SkilmeAI.GameOS.csproj`；所有 `SkilmeAI/**/*.cs` 编译进游戏主 assembly，`ScriptPathAttribute` 基于游戏 `GodotProjectDir` 生成，脚本和场景加载链路完全成立。
