...
当前通过 `git submodule` 嵌入框架源码：

```text
framework_repo:      /home/slime/Code/SlimeAI/SlimeAI
submodule_path:      /home/slime/Code/SlimeAI/Games/BrotatoLike/SlimeAI
submodule_remote:    https://github.com/alading-2/SlimeAIGameFramework.git
submodule_status:    +6eeb6ee7cd292ed1c41ef9757459cb954bef83ea (heads/main)
framework_project:   SlimeAI/GameOS/SlimeAI.GameOS.csproj (submodule 内)
game_csproj:         BrotatoLike.csproj
engine_source_path:  /home/slime/Code/SlimeAI/Resources/Engine/godot-4.6.2-stable
source_input_repo:   /home/slime/Code/Godot/Games/MyGames/brotato-my
copied_assets:       /home/slime/Code/SlimeAI/Games/BrotatoLike/assets
migration_input:     /home/slime/Code/SlimeAI/Games/BrotatoLike/MigrationInput
```

## 规则

- 游戏任务默认只读取框架契约和 API 索引。
- 框架源码修改必须切换到框架仓库（`SlimeAI/` 是只读镜像，禁止在游戏仓直接改动）。
- 框架升级通过 `git submodule update` 拉取，升级后必须在本仓库跑构建和游戏回归。

## 当前框架状态

- `SlimeAI.GameOS` 已可 build，并包含 Data / Event / Entity / Relationship / Schedule / Resource / Pool / Timer Runtime 最小内核、Movement Capability 旧 `MoveMode` 纯 C# 策略 + Godot 2D 位移桥 + MovementCollision + 同帧多命中 + Godot Physics broadphase + Godot Orientation、Collision Capability 纯运行时第一批、Damage / ContactDamage / Damage 处理器管线 / HealService / DamageTool 第一批、Ability Runtime 最小切片 + 点选目标语义 + 自动索敌第一段 + Periodic 自动触发 Tick、Projectile / Effect Runtime 生成第一段、Projectile 命中生命周期、穿透 / 生命周期扩展、Effect 动画播放第一段和 Godot 实例化第一段、Feature Runtime 最小生命周期、AI Runtime 最小行为树 + 最近目标查询 + 巡逻 + 行为树预制块 + Ability 自动索敌上下文准备 + Godot AI bridge + 攻击请求事件、Attack Runtime 最小结算、GodotAttackComponent bridge 第一段、旧 AttackComponent 兼容包装、GodotUnitAnimationComponent 动画事件桥第一段和 GodotBridge 第一版。
- 游戏 csproj 通过 `ProjectReference` 引用 submodule 内 `SlimeAI.GameOS.csproj`；所有 `SlimeAI/**/*.cs` 编译进游戏主 assembly，`ScriptPathAttribute` 基于游戏 `GodotProjectDir` 生成，脚本和场景加载链路完全成立。
